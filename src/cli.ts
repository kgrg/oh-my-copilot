...
async function snapshotSessionsForReview(): Promise<string[]> {
  try {
...
async function maybeTriggerHeadlessReview(argv: string[], cwd: string, before: string[]): Promise<void> {
  try {
...
export async function runCli(argv = process.argv.slice(2)): Promise<CliResult> {
  ...
  if (group === "setup") {
    ...
    const baseOpts = {
      ...
      scope: (flagValue(argv, "--scope") === "user" ? "user" : "project") as "user" | "project",
    };
    const forceFlag = hasFlag(argv, "--force");
    let result = runSetup({ ...baseOpts, force: forceFlag });
    // Offer to override bundled skills/agents that exist locally but differ
    // (so skill updates actually land). Skipped under --force / --dry-run / --json.
    if (!forceFlag && !baseOpts.dryRun && isInteractive(json)) {
      const changed = result.actions.filter((a) => a.kind === "skip-changed").map((a) => a.target);
      if (changed.length > 0) {
        const readline = await import("node:readline/promises");
        const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
        try {
          console.log(`${changed.length} bundled file(s) differ from your local copies:`);
          for (const c of changed.slice(0, 20)) console.log(`  ${c}`);
          if (changed.length > 20) console.log(`  …and ${changed.length - 20} more`);
          const answer = ((await rl.question("Override them with the bundled versions? [y/N] ")) ?? "")
            .trim()
            .toLowerCase();
          if (answer === "y" || answer === "yes") result = runSetup({ ...baseOpts, force: true });
        } finally {
          rl.close();
        }
      }
  ...
  if (group === "ponytail") {
    ...
    if (command === "status") {
      const state = readPonytail(cwd);
      return json
        ? { ok: true, output: state ?? { active: false } }
        : { ok: true, message: state?.active ? `ponytail active: ${state.level}` : "ponytail inactive" };
    }
    if (command === "off" || command === "cancel") {
      cancelPonytail(cwd);
      return json ? { ok: true, output: { cancelled: true } } : { ok: true, message: "ponytail off" };
    }
    return { ok: false, exitCode: 1, message: "Unknown ponytail subcommand. Try: ponytail start [lite|full|ultra] | status | off" };
  }
  ...
  if (group === "config") {
    const { readMemoryConfig, setMemoryConfigValue, unsetMemoryConfigValue } = await import("./memory-review/config.js");
    const cwd = flagValue(argv, "--root") ?? process.cwd();
    ...
    if (command === "set") {
      ...
      if (value === "memory-mode") {
        ...
        if (setVal === "off") {
          ...
          // Drop any stale PROJECT memoryMode so it can't shadow the global off.
...
async function handleScheduleCommand(argv: string[], json: boolean): Promise<CliResult> {
  ...
  if (command === "open" && targetId) {
    ...
    if (json) return { ok: true, output: r };
    const j = r.job!;
